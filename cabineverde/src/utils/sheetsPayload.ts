import type { CasoCompleto, SheetCellValue, SheetPayload } from '../types/case.js';

const colunasDesaparecidosBase = [
  'idCaso',
  'dataHoraRegistro',
  'dataServico',
  'turno',
...
const colunasArvoreDesaparecidos = [
...
const colunasDesaparecidos = [...colunasDesaparecidosBase, ...colunasArvoreDesaparecidos] as const;
...
const normalizarRespostaArvore = (valor: unknown): string => {
  if (typeof valor === 'boolean') {
    return valor ? 'Sim' : 'Não';
  }

  const valorTexto = String(valor ?? '').trim();
  return valorTexto || 'Não informado';
};

const normalizarComplementoArvore = (valor: unknown): string => String(valor ?? '').trim();

const mapearDadosArvore = (caso: CasoCompleto): Record<string, SheetCellValue> => {
  const subfluxo = caso.subfluxoPerguntas || {};

  return colunasArvoreDesaparecidos.reduce<Record<string, SheetCellValue>>((acumulado, coluna) => {
    const valor = subfluxo[coluna];
    acumulado[coluna] = coluna.endsWith('_resp') ? normalizarRespostaArvore(valor) : normalizarComplementoArvore(valor);
    return acumulado;
  }, {});
};
